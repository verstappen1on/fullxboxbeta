// ==UserScript==
// @name         fullpass
// @namespace    https://github.com/verstappen1on
// @version      1.0
// @description  Improve Xbox Cloud Gaming (xCloud) experience
// @author       verstappen1on
// @license      MIT
// @match        https://www.xbox.com/*/play*
// @match        https://www.xbox.com/*/auth/msa?*loggedIn*
// @run-at       document-start
// @grant        none
// @updateURL    https://raw.githubusercontent.com/verstappen1on/fullpass/main/fullpass.meta.js
// @downloadURL  https://github.com/verstappen1on/fullpass/releases/latest/download/fullpass.user.js
// ==/UserScript==
'use strict';

const SCRIPT_VERSION = '1.0';
const SCRIPT_HOME = 'https://github.com/verstappen1on/fullpass';

const ENABLE_XCLOUD_LOGGER = false;
const ENABLE_PRELOAD_fp_UI = false;
...
if (window.location.pathname.includes('/auth/msa')) {
    ...
    throw new Error('[fullpass] Refreshing the page after logging in');
}

console.log(`[fullpass] readyState: ${document.readyState}`);

const fpEvent = {
    JUMP_BACK_IN_READY: 'fp-jump-back-in-ready',
    POPSTATE: 'fp-popstate',

    STREAM_LOADING: 'fp-stream-loading',
    STREAM_STARTING: 'fp-stream-starting',
    STREAM_STARTED: 'fp-stream-started',
    STREAM_PLAYING: 'fp-stream-playing',
    STREAM_STOPPED: 'fp-stream-stopped',
    STREAM_ERROR_PAGE: 'fp-stream-error-page',

    STREAM_MENU_SHOWN: 'fp-stream-menu-shown',
    STREAM_MENU_HIDDEN: 'fp-stream-menu-hidden',

    STREAM_WEBRTC_CONNECTED: 'fp-stream-webrtc-connected',
    STREAM_WEBRTC_DISCONNECTED: 'fp-stream-webrtc-disconnected',

    CUSTOM_TOUCH_LAYOUTS_LOADED: 'fp-custom-touch-layouts-loaded',

    DATA_CHANNEL_CREATED: 'fp-data-channel-created',

    dispatch: (target, eventName, data) => {
        if (!eventName) {
            alert('fpEvent.dispatch(): eventName is null');
            return;
        }
...
const ButtonStyle = {};
ButtonStyle[ButtonStyle.PRIMARY = 1] = 'fp-primary';
ButtonStyle[ButtonStyle.DANGER = 2] = 'fp-danger';
ButtonStyle[ButtonStyle.GHOST = 4] = 'fp-ghost';
ButtonStyle[ButtonStyle.FOCUSABLE = 8] = 'fp-focusable';
ButtonStyle[ButtonStyle.FULL_WIDTH = 16] = 'fp-full-width';
ButtonStyle[ButtonStyle.FULL_HEIGHT = 32] = 'fp-full-height';

const ButtonStyleIndices = Object.keys(ButtonStyle).splice(0, Object.keys(ButtonStyle).length / 2).map(i => parseInt(i));


const createButton = options => {
    const $btn = CE(options.url ? 'a' : 'button', {'class': 'fp-button'});
...
const Translations = {
    getLocale: () => {
        ...
        let locale = localStorage.getItem('full_pass_locale');
        if (!locale) {
    ...
    get: (key, values) => {
        ...
        const translation = texts[LOCALE] || texts['pt-BR'];
    ...
    "safari-failed-message": {
        "de-DE": "Ausführen von \"fullpass\" fehlgeschlagen. Versuche es erneut, bitte warten...",
        "en-US": "Failed to run fullpass. Retrying, please wait...",
        "es-ES": "No se pudo ejecutar fullpass. Reintentando, por favor espera...",
        "fr-FR": "Impossible d'exécuter fullpass. Nouvelle tentative, veuillez patienter...",
        "it-IT": "Si è verificato un errore durante l'esecuzione di fullpass. Nuovo tentativo, attendere...",
        "ja-JP": "fullpass の実行に失敗しました。再試行中...",
        "ko-KR": "fullpass 시작에 실패했습니다. 재시도중이니 잠시만 기다려 주세요.",
        "pl-PL": "Nie udało się uruchomić fullpass. Ponawiam próbę...",
        "pt-BR": "Falha ao executar o fullpass. Tentando novamente, aguarde...",
        "ru-RU": "Не удалось запустить fullpass. Идет перезапуск, пожалуйста, подождите...",
        "tr-TR": "fullpass çalıştırılamadı. Yeniden deneniyor...",
        "uk-UA": "Не вдалий старт fullpass. Повторна спроба, будь ласка, зачекайте...",
        "vi-VN": "Không thể chạy fullpass. Đang thử lại, vui lòng chờ...",
    ...
    "support-fullpass": {
        "de-DE": "\"fullpass\" unterstützen",
        "en-US": "Support fullpass",
        "es-ES": "Apoyar a fullpass",
        "ja-JP": "fullpassをサポート",
        "pl-PL": "Wesprzyj fullpass",
        "pt-BR": "Suporte ao Melhor xCloud",
        "ru-RU": "Поддержать fullpass",
        "tr-TR": "fullpass'a destek ver",
        "uk-UA": "Підтримати fullpass",
        "vi-VN": "Ủng hộ fullpass",
        "zh-CN": "赞助本插件",
    },
    "swap-buttons": {
...
`;
    ...
    $fragment.appendChild(CE('div', {'class': 'fp-reload-overlay'}, __('safari-failed-message')));
    ...
    // Stop processing the script
    throw new Error('[fullpass] Executing workaround for Safari');
}
...
class Dialog {
    constructor(options) {
        ...
        this.$overlay = document.querySelector('.fp-dialog-overlay');
        if (!this.$overlay) {
            this.$overlay = CE('div', {'class': 'fp-dialog-overlay fp-gone'});
        ...
        this.$dialog = CE('div', {'class': `fp-dialog ${className || ''} fp-gone`},
                ...
                this.$content = CE('div', {'class': 'fp-dialog-content'}, content),
        ...
        !title && this.$title.classList.add('fp-gone');
        !content && this.$content.classList.add('fp-gone');
    ...
    show(newOptions) {
        ...
        if (newOptions && newOptions.title) {
            ...
            this.$title.classList.remove('fp-gone');
        }

        this.$dialog.classList.remove('fp-gone');
        this.$overlay.classList.remove('fp-gone');

        document.body.classList.add('fp-no-scroll');
    }

    hide(e) {
        this.$dialog.classList.add('fp-gone');
        this.$overlay.classList.add('fp-gone');
    ...
    toggle() {
        this.$dialog.classList.toggle('fp-gone');
        this.$overlay.classList.toggle('fp-gone');
    }
}
...
class RemotePlay {
    ...
    static #initialize() {
        ...
        RemotePlay.#dialog = new Dialog({
            ...
            helpUrl: 'https://fullpass.github.io/remote-play/',
        });
    ...
    static #renderConsoles() {
        ...
        if (!RemotePlay.#CONSOLES || RemotePlay.#CONSOLES.length === 0) {
            ...
            const $qualitySettings = CE('div', {'class': 'fp-remote-play-settings'},
                CE('div', {},
        ...
        for (let con of RemotePlay.#CONSOLES) {
            let $connectButton;
            const $child = CE('div', {'class': 'fp-remote-play-device-wrapper'},
                CE('div', {'class': 'fp-remote-play-device-info'},
                    CE('div', {},
                        CE('span', {'class': 'fp-remote-play-device-name'}, con.deviceName),
                        CE('span', {'class': 'fp-remote-play-console-type'}, con.consoleType.replace('Xbox', ''))
                    ),
                    CE('div', {'class': 'fp-remote-play-power-state'}, RemotePlay.#STATE_LABELS[con.powerState]),
                ),

                // Connect button
                createButton({
                    classes: ['fp-remote-play-connect-button'],
                    ...
                    onClick: e => {
                            ...
                            window.fp_REMOTE_PLAY_CONFIG = REMOTE_PLAY_CONFIG;

                            const url = window.location.href.substring(0, 31) + '/launch/fortnite/BT5P2X999VH2#remote-play';

                            const $pageContent = document.getElementById('PageContent');
                            const $anchor = CE('a', { href: url, class: 'fp-hidden fp-offscreen' }, '');
    ...
    static detect() {
        ...
        if (IS_REMOTE_PLAYING) {
            ...
            window.history.replaceState({origin: 'fullpass'}, '', 'https://www.xbox.com/' + location.pathname.substring(1, 6) + '/play');
        } else {
            window.fp_REMOTE_PLAY_CONFIG = null;
        }
    }
...
`;
    ...
    static setupWaitTime(waitTime) {
        ...
        if (!$waitTimeBox) {
            $waitTimeBox = CE('div', {'class': 'fp-wait-time-box'},
                                    CE('label', {}, __('server')),
            ...
            $waitTimeBox.classList.remove('fp-gone');
            $estimated = $waitTimeBox.querySelector('.fp-wait-time-estimated');
            $countDown = $waitTimeBox.querySelector('.fp-wait-time-countdown');
        }
    ...
    static hide() {
        ...
        LoadingScreen.#$waitTimeBox && LoadingScreen.#$waitTimeBox.classList.add('fp-gone');
...
class TouchController {
    ...
    static #show() {
        document.querySelector('#BabylonCanvasContainer-main').parentElement.classList.remove('fp-offscreen');
        TouchController.#showing = true;
    }

    static #hide() {
        document.querySelector('#BabylonCanvasContainer-main').parentElement.classList.add('fp-offscreen');
    ...
    static #dispatchLayouts(data) {
        fpEvent.dispatch(window, fpEvent.CUSTOM_TOUCH_LAYOUTS_LOADED, {
            data: data,
        });
    };
    ...
    static async getCustomLayouts(xboxTitleId, retries) {
        ...
        if (retries > 2) {
            ...
            // Wait for fp_EXPOSED.touch_layout_manager
            setTimeout(() => TouchController.#dispatchLayouts(null), 1000);
            return;
        }

        const baseUrl = `https://raw.githubusercontent.com/verstappen1on/fullpass/gh-pages/touch-layouts${USE_DEV_TOUCH_LAYOUT ? '/dev' : ''}`;
    ...
    static loadCustomLayout(xboxTitleId, layoutId, delay) {
        if (!window.fp_EXPOSED.touch_layout_manager) {
            return;
        }
        ...
        if (layout) {
            ...
            setTimeout(() => {
                window.fp_EXPOSED.touch_layout_manager.changeLayoutForScope({
                    type: 'showLayout',
                    scope: xboxTitleId,
                    subscope: 'base',
                    layout: {
    ...
    static setup() {
        ...
        window.fp_EXPOSED.test_touch_control = content => {
            const { touch_layout_manager } = window.fp_EXPOSED;
        ...
        const $bar = createElement('div', {'id': 'fp-touch-controller-bar'});
        ...
        window.addEventListener(fpEvent.DATA_CHANNEL_CREATED, e => {
            ...
            dataChannel.addEventListener('message', msg => {
                if (msg.origin === 'fullpass' || typeof msg.data !== 'string') {
                    return;
                }
...
class Toast {
    ...
    static show(msg, status) {
        ...
        if (status) {
            Toast.#$status.classList.remove('fp-gone');
            Toast.#$status.textContent = status;
        } else {
            Toast.#$status.classList.add('fp-gone');
        }

        const classList = Toast.#$wrapper.classList;
        classList.remove('fp-offscreen', 'fp-hide');
        classList.add('fp-show');
    }
    ...
    static #hide() {
        ...
        const classList = Toast.#$wrapper.classList;
        classList.remove('fp-show');
        classList.add('fp-hide');
    }

    static setup() {
        Toast.#$wrapper = CE('div', {'class': 'fp-toast fp-offscreen'},
                                        Toast.#$msg = CE('span', {'class': 'fp-toast-msg'}),
                                        Toast.#$status = CE('span', {'class': 'fp-toast-status'}));

        Toast.#$wrapper.addEventListener('transitionend', e => {
            const classList = Toast.#$wrapper.classList;
            if (classList.contains('fp-hide')) {
                classList.remove('fp-offscreen', 'fp-hide');
                classList.add('fp-offscreen');
            }
        });
...
class SettingElement {
    ...
    static #renderNumberStepper(key, setting, value, onChange, options={}) {
        ...
        const $wrapper = CE('div', {'class': 'fp-number-stepper'},
        ...
        if (options.disabled) {
            ...
            $incBtn.classList.add('fp-hidden');

            $decBtn.disabled = true;
            $decBtn.classList.add('fp-hidden');
            return $wrapper;
        }
...
class LocalDb {
    ...
    static get DB_NAME() { return 'fullpass'; }
...
class MkbHandler {
    ...
    #waitForPointerLock = (wait) => {
        this.#$message && this.#$message.classList.toggle('fp-gone', !wait);
    }
    ...
    init = () => {
        ...
        this.#$message = CE('div', {'class': 'fp-mkb-pointer-lock-msg fp-gone'},
                createButton({
        ...
        window.addEventListener(fpEvent.STREAM_MENU_SHOWN, this.#onStreamMenuShown);
        window.addEventListener(fpEvent.STREAM_MENU_HIDDEN, this.#onStreamMenuHidden);
    ...
    destroy = () => {
        ...
        window.removeEventListener(fpEvent.STREAM_MENU_SHOWN, this.#onStreamMenuShown);
        window.removeEventListener(fpEvent.STREAM_MENU_HIDDEN, this.#onStreamMenuHidden);
    }
    ...
    start = () => {
        ...
        fpEvent.dispatch(window, 'gamepadconnected', {
    ...
    stop = () => {
        ...
        fpEvent.dispatch(window, 'gamepaddisconnected', {
                gamepad: virtualGamepad,
            });
    ...
    static setupEvents() {
        window.addEventListener(fpEvent.STREAM_PLAYING, e => {
            // Enable MKB
...
class MkbRemapper {
    ...
    #toggleEditing = force => {
        ...
        this.#$.wrapper.classList.toggle('fp-editing', this.#STATE.isEditing);
        ...
        for (const $elm of childElements) {
            if ($elm.parentElement.parentElement.classList.contains('fp-mkb-action-buttons')) {
                continue;
            }

            let disable = !this.#STATE.isEditing;

            if ($elm.parentElement.classList.contains('fp-mkb-preset-tools')) {
                disable = !disable;
            }
    ...
    render() {
        this.#$.wrapper = CE('div', {'class': 'fp-mkb-settings'});
        ...
        const $header = CE('div', {'class': 'fp-mkb-preset-tools'},
                this.#$.presetsSelect,
                // Rename button
                createButton({
                    title: __('rename'),
        ...
        const $rows = CE('div', {'class': 'fp-mkb-settings-rows'},
                CE('i', {'class': 'fp-mkb-note'}, __('right-click-to-unbind')),
            );
        ...
        for (const buttonIndex of this.#BUTTON_ORDERS) {
            ...
            const $keyRow = CE('div', {'class': 'fp-mkb-key-row'},
        ...
        $rows.appendChild(CE('i', {'class': 'fp-mkb-note'}, __('mkb-adjust-ingame-settings')),);
        ...
        for (const key in MkbPreset.MOUSE_SETTINGS) {
            ...
            const $row = CE('div', {'class': 'fp-quick-settings-row'},
                    CE('label', {'for': `fp_setting_${key}`}, setting.label),
        ...
        const $actionButtons = CE('div', {'class': 'fp-mkb-action-buttons'},
                CE('div', {},
                   // Edit button
                   createButton({
                           label: __('edit'),
...
class VibrationManager {
    static #playDeviceVibration(data) {
        ...
        const intensity = Math.min(100, data.leftMotorPercent + data.rightMotorPercent / 2) * window.fp_VIBRATION_INTENSITY;
    ...
    static updateGlobalVars() {
        window.fp_ENABLE_CONTROLLER_VIBRATION = VibrationManager.supportControllerVibration() ? getPref(Preferences.CONTROLLER_ENABLE_VIBRATION) : false;
        window.fp_VIBRATION_INTENSITY = getPref(Preferences.CONTROLLER_VIBRATION_INTENSITY) / 100;

        if (!VibrationManager.supportDeviceVibration()) {
            window.fp_ENABLE_DEVICE_VIBRATION = false;
            return;
        }
        ...
        window.fp_ENABLE_DEVICE_VIBRATION = enabled;
    }
    ...
    static initialSetup() {
        ...
        window.addEventListener(fpEvent.DATA_CHANNEL_CREATED, e => {
            ...
            dataChannel.addEventListener('message', e => {
                if (!window.fp_ENABLE_DEVICE_VIBRATION) {
                    return;
                }
...
class StreamBadges {
    ...
    static #renderBadge(name, value, color) {
        ...
        $badge = CE('div', {'class': 'fp-badge'},
                    CE('span', {'class': 'fp-badge-name'}, __(`badge-${name}`)),
                    CE('span', {'class': 'fp-badge-value', 'style': `background-color: ${color}`}, value));

        if (name === StreamBadges.BADGE_BATTERY) {
            $badge.classList.add('fp-badge-battery');
        }
    ...
    static async #updateBadges(forceUpdate) {
        if (!forceUpdate && !document.querySelector('.fp-badges')) {
            StreamBadges.#stop();
            return;
        }
    ...
    static async render() {
        ...
        const $wrapper = createElement('div', {'class': 'fp-badges'});
...
class StreamStats {
    ...
    static start(glancing=false) {
        ...
        StreamStats.#$container.classList.remove('fp-gone');
    ...
    static stop(glancing=false) {
        ...
        if (StreamStats.#$container) {
            ...
            StreamStats.#$container.classList.add('fp-gone');
        }
    }
    ...
    static isHidden = () => StreamStats.#$container && StreamStats.#$container.classList.contains('fp-gone');
    ...
    static render() {
        ...
        for (let statKey in STATS) {
            const $div = CE('div', {'class': `fp-stat-${statKey}`, title: STATS[statKey][0]}, CE('label', {}, statKey.toUpperCase()), STATS[statKey][1]);
            $barFragment.appendChild($div);
        }

        StreamStats.#$container = CE('div', {'class': 'fp-stats-bar fp-gone'}, $barFragment);
...
class Preferences {
    ...
    static get BETTER_XCLOUD_LOCALE() { return 'fp_locale'};
    ...
    static SETTINGS = {
        ...
        [Preferences.CONTROLLER_SUPPORT_LOCAL_CO_OP]: {
            'default': true,
            'note':CE('a', {
                           href: 'https://github.com/verstappen1on/fullpass/discussions/275',
                           target: '_blank',
        ...
        [Preferences.MKB_ENABLED]: {
            ...
            'ready': () => {
                ...
                if (pref.unsupported) {
                    ...
                    url = 'https://github.com/verstappen1on/fullpass/issues/206#issuecomment-1920475657';
                } else {
                    note = __('mkb-disclaimer');
                    url = 'https://fullpass.github.io/mouse-and-keyboard/#disclaimer';
                }
...
class Patcher {
    static #PATCHES = {
        ...
        remotePlayConnectMode: function(funcStr) {
            ...
            return funcStr.replace(text, `connectMode:window.fp_REMOTE_PLAY_CONFIG?"xhome-connect":"cloud-connect",remotePlayServerId:(window.fp_REMOTE_PLAY_CONFIG&&window.fp_REMOTE_PLAY_CONFIG.serverId)||''`);
        },
        ...
        playVibration: function(funcStr) {
            ...
            const newCode = `
if (!window.fp_ENABLE_CONTROLLER_VIBRATION) {
    return void(0);
}
if (window.fp_VIBRATION_INTENSITY && window.fp_VIBRATION_INTENSITY < 1) {
    e.leftMotorPercent = e.leftMotorPercent * window.fp_VIBRATION_INTENSITY;
    e.rightMotorPercent = e.rightMotorPercent * window.fp_VIBRATION_INTENSITY;
    e.leftTriggerMotorPercent = e.leftTriggerMotorPercent * window.fp_VIBRATION_INTENSITY;
    e.rightTriggerMotorPercent = e.rightTriggerMotorPercent * window.fp_VIBRATION_INTENSITY;
}
`;
        ...
        loadingEndingChunks: function(funcStr) {
            ...
            console.log('[fullpass] Remaining patches:', Patcher.#PATCH_ORDERS);
        ...
        exposeTouchLayoutManager: function(funcStr) {
            ...
            funcStr = funcStr.replace(text, 'window.fp_EXPOSED["touch_layout_manager"] = this,' + text);
            return funcStr;
        },
...
let match = onGamepadInputStr.match(/(\\w+\\.GamepadIndex)/);

if (match) {
    const gamepadIndexVar = match[0];
    onGamepadInputStr = onGamepadInputStr.replace('this.gamepadStates.get(', \`this.gamepadStates.get(\${gamepadIndexVar},\`);
    eval(\`this.onGamepadInput = function \${onGamepadInputStr}\`);
    console.log('[fullpass] Successfully patched local co-op support');
} else {
    console.error('[fullpass] Unable to patch local co-op support');
}
    ...
    static #patchFunctionBind() {
        ...
        Function.prototype.bind = function() {
            ...
            if (typeof arguments[1] === 'function') {
                console.log('[fullpass] Restored Function.prototype.bind()');
        ...
        for (let id in item[1]) {
            ...
            for (let groupIndex = 0; groupIndex < Patcher.#PATCH_ORDERS.length; groupIndex++) {
                ...
                for (let patchIndex = 0; patchIndex < group.length; patchIndex++) {
                    ...
                    console.log(`[fullpass] Applied "${patchName}" patch`);
...
function checkForUpdate() {
    ...
    fetch('https://api.github.com/repos/verstappen1on/fullpass/releases/latest')
...
function addCss() {
    let css = `
:root {
    --fp-title-font: Bahnschrift, Arial, Helvetica, sans-serif;
    --fp-title-font-semibold: Bahnschrift Semibold, Arial, Helvetica, sans-serif;
    --fp-normal-font: "Segoe UI", Arial, Helvetica, sans-serif;
    --fp-monospaced-font: Consolas, "Courier New", Courier, monospace;
    --fp-promptfont-font: promptfont;

    --fp-button-height: 36px;

    --fp-default-button-color: #2d3036;
    --fp-default-button-hover-color: #515863;
    --fp-default-button-disabled-color: #8e8e8e;

    --fp-primary-button-color: #008746;
    --fp-primary-button-hover-color: #04b358;
    --fp-primary-button-disabled-color: #448262;

    --fp-danger-button-color: #c10404;
    --fp-danger-button-hover-color: #e61d1d;
    --fp-danger-button-disabled-color: #a26c6c;

    --fp-toast-z-index: 9999;
    --fp-dialog-z-index: 9101;
    --fp-dialog-overlay-z-index: 9100;
    --fp-stats-bar-z-index: 9001;
    --fp-stream-settings-z-index: 9000;
    --fp-mkb-pointer-lock-msg-z-index: 8999;
    --fp-screenshot-z-index: 8888;
    --fp-touch-controller-bar-z-index: 5555;
    --fp-wait-time-box-z-index: 100;
}

@font-face {
    font-family: 'promptfont';
    src: url('https://verstappen1on.github.io/fullpass/fonts/promptfont.otf');
}
...
.fp-button {
    background-color: var(--fp-default-button-color);
    user-select: none;
    -webkit-user-select: none;
    color: #fff;
    font-family: var(--fp-title-font-semibold);
    font-size: 14px;
    border: none;
    font-weight: 400;
    height: var(--fp-button-height);
    border-radius: 4px;
    padding: 0 8px;
...
.fp-button:hover, .fp-button.fp-focusable:focus {
    background-color: var(--fp-default-button-hover-color);
}

.fp-button:disabled {
    cursor: default;
    background-color: var(--fp-default-button-disabled-color);
}
...
.fp-button.fp-ghost:hover, .fp-button.fp-ghost.fp-focusable:focus {
    background-color: var(--fp-default-button-hover-color);
}

.fp-button.fp-primary {
    background-color: var(--fp-primary-button-color);
}

.fp-button.fp-primary:hover, .fp-button.fp-primary.fp-focusable:focus {
    background-color: var(--fp-primary-button-hover-color);
}

.fp-button.fp-primary:disabled {
    background-color: var(--fp-primary-button-disabled-color);
}

.fp-button.fp-danger {
    background-color: var(--fp-danger-button-color);
}

.fp-button.fp-danger:hover, .fp-button.fp-danger.fp-focusable:focus {
    background-color: var(--fp-danger-button-hover-color);
}

.fp-button.fp-danger:disabled {
    background-color: var(--fp-danger-button-disabled-color);
}
...
.fp-button svg:not(:only-child) {
    margin-right: 4px;
}

.fp-button span {
    display: inline-block;
    height: calc(var(--fp-button-height) - 2px);
    line-height: var(--fp-button-height);
...
.fp-remote-play-button {
    height: auto;
    margin-right: 8px !important;
}

.fp-remote-play-button svg {
    width: 28px;
    height: 46px;
}
...
.fp-settings-button[data-update-available]::after {
    content: ' 🌟';
}

.fp-remote-play-button, .fp-settings-button {
    position: relative;
}

.fp-remote-play-button::after, .fp-settings-button::after {
    border: 2px solid transparent;
    border-radius: 4px;
}

.fp-remote-play-button:focus::after, .fp-settings-button:focus::after {
    content: '';
    border-color: white;
    position: absolute;
    top: 0;
...
.better_xcloud_settings {
    ...
    -webkit-user-select: none;
    color: #fff;
    font-family: var(--fp-normal-font);
}
...
.fp-settings-wrapper *:focus {
    outline: none !important;
}

.fp-settings-wrapper .fp-settings-title-wrapper {
    display: flex;
    margin-bottom: 10px;
    align-items: center;
}

.fp-settings-wrapper a.fp-settings-title {
...
@media (hover: hover) {
    .fp-settings-wrapper a.fp-settings-title:hover {
        color: #83f73a;
    }
}

.fp-settings-wrapper a.fp-settings-title:focus {
    color: #83f73a;
}

.fp-settings-wrapper a.fp-settings-update {
    display: block;
    color: #ff834b;
    text-decoration: none;
    margin-bottom: px;
...
@media (hover: hover) {
    .fp-settings-wrapper a.fp-settings-update:hover {
        color: #ff9869;
        text-decoration: underline;
    }
}

.fp-settings-wrapper a.fp-settings-update:focus {
    color: #ff9869;
    text-decoration: underline;
}
...
.fp-settings-group-label b, .fp-settings-row label b {
    display: block;
    font-size: 12px;
    font-style: italic;
    font-weight: normal;
...
@media not (hover: hover) {
    .fp-settings-row:focus-within {
       background-color: #242424;
    }
}

.fp-settings-row input {
    align-self: center;
}

.fp-settings-wrapper .fp-button.fp-primary {
    margin-top: 8px;
}

.fp-settings-app-version {
    margin-top: 10px;
    text-align: center;
    color: #747474;
    font-size: 12px;
...
.fp-donation-link:hover {
    color: #6dd72b;
}

.fp-settings-custom-user-agent {
    display: block;
    width: 100%;
}
...
.fp-badge-battery[data-charging=true] span:first-of-type::after {
    content: ' ⚡️';
}

.fp-screenshot-button {
    display: none;
    opacity: 0;
    position: fixed;
    bottom: 0;
    ...
    z-index: var(--fp-screenshot-z-index);

    /* Credit: https://phosphoricons.com */
    background-image: url(${Icon.SCREENSHOT_B64});
}

.fp-screenshot-button[data-showing=true] {
    opacity: 0.9;
}

.fp-screenshot-button[data-capturing=true] {
    padding: 1vh;
}

.fp-screenshot-canvas {
    display: none;
}
...
.fp-stats-bar {
    ...
    color: #fff;
    font-family: var(--fp-monospaced-font);
    font-size: 0.9rem;
    padding-left: 8px;
    z-index: var(--fp-stats-bar-z-index);
    text-wrap: nowrap;
}
...
.fp-stats-bar[data-stats*="[fps]"] > .fp-stat-fps,
.fp-stats-bar[data-stats*="[ping]"] > .fp-stat-ping,
.fp-stats-bar[data-stats*="[btr]"] > .fp-stat-btr,
.fp-stats-bar[data-stats*="[dt]"] > .fp-stat-dt,
.fp-stats-bar[data-stats*="[pl]"] > .fp-stat-pl,
.fp-stats-bar[data-stats*="[fl]"] > .fp-stat-fl {
    display: inline-block;
}
...
.fp-stats-bar[data-display=glancing]::before {
    content: '👀 ';
    vertical-align: middle;
}

.fp-stats-bar[data-position=top-left] {
    left: 0;
    border-radius: 0 0 4px 0;
}

.fp-stats-bar[data-position=top-right] {
    right: 0;
    border-radius: 0 0 0 4px;
}

.fp-stats-bar[data-position=top-center] {
    transform: translate(-50%, 0);
    left: 50%;
    border-radius: 0 0 4px 4px;
}

.fp-stats-bar[data-transparent=true] {
    background: none;
...
.fp-stats-bar span[data-grade=good] {
    color: #6bffff;
}

.fp-stats-bar span[data-grade=ok] {
    color: #fff16b;
}

.fp-stats-bar span[data-grade=bad] {
    color: #ff5f5f;
}

.fp-stats-bar span:first-of-type {
    min-width: 22px;
}
...
.fp-dialog-overlay {
    position: fixed;
    inset: 0;
    z-index: var(--fp-dialog-overlay-z-index);
    background: black;
    opacity: 50%;
}
...
.fp-dialog.fp-binding-dialog h2 b {
    font-family: var(--fp-promptfont-font) !important;
}
...
@media (hover: hover) {
    .fp-dialog > button:hover {
        background-color: #515863;
    }
}

.fp-dialog > button:focus {
    background-color: #515863;
}

.fp-stats-settings-dialog > div > div {
    display: flex;
    margin-bottom: 8px;
    padding: 2px 4px;
}

.fp-stats-settings-dialog label {
    flex: 1;
    margin-bottom: 0;
    align-self: center;
}

.fp-quick-settings-bar {
    display: flex;
    position: fixed;
    z-index: var(--fp-stream-settings-z-index);
    opacity: 0.98;
    user-select: none;
    -webkit-user-select: none;
}

.fp-quick-settings-tabs {
    position: fixed;
    top: 0;
    right: 420px;
    display: flex;
...
.fp-quick-settings-tabs svg.fp-active {
    background: #222;
    border-color: #008746;
}

.fp-quick-settings-tabs svg:not(.fp-active):hover {
    background: #2f2f2f;
    border-color: #484848;
}

.fp-quick-settings-tab-contents {
    flex-direction: column;
    position: fixed;
    right: 0;
    top: 0;
...
.fp-quick-settings-tab-contents > div[data-group=mkb] {
    display: flex;
    flex-direction: column;
    height: 100%;
    overflow: hidden;
...
.fp-quick-settings-bar:not([data-clarity-boost="true"]) .fp-clarity-boost-warning {
    display: none;
}

.fp-quick-settings-bar[data-clarity-boost="true"] .fp-clarity-boost-warning {
    display: block;
    margin: 0px 8px;
    padding: 12px;
    font-size: 16px;
...
.fp-quick-settings-bar[data-clarity-boost="true"] div[data-type="video"] {
    display: none;
}

.fp-quick-settings-tab-contents *:focus {
    outline: none !important;
}

.fp-quick-settings-row {
    display: flex;
...
.fp-quick-settings-tab-contents h2 {
    margin-bottom: 8px;
    display: flex;
    align-item: center;
}

.fp-quick-settings-tab-contents h2 span {
    display: inline-block;
    font-size: 24px;
    font-weight: bold;
    text-transform: uppercase;
    ...
    flex: 1;
    height: var(--fp-button-height);
    line-height: calc(var(--fp-button-height) + 4px);
    text-overflow: ellipsis;
    overflow: hidden;
    white-space: nowrap;
}

.fp-quick-settings-tab-contents input[type="range"] {
    display: block;
...
@media (hover: hover) {
    .fp-number-stepper button:hover {
        background-color: var(--fp-default-button-hover-color);
    }
}

.fp-number-stepper button:active {
    background-color: var(--fp-default-button-hover-color);
}

.fp-number-stepper input[type=range]:disabled, .fp-number-stepper button:disabled {
    display: none;
}

.fp-number-stepper button:disabled + span {
    font-family: var(--fp-title-font);
}
...
.fp-mkb-settings select:disabled {
    background: transparent;
    border: none;
    color: #fff;
}

.fp-quick-settings-row select:disabled {
    text-align: right;
}

.fp-mkb-pointer-lock-msg {
    display: flex;
    cursor: pointer;
    user-select: none;
    -webkit-user-select: none;
    ...
    margin: auto;
    background: #000000e5;
    z-index: var(--fp-mkb-pointer-lock-msg-z-index);
    color: #fff;
    text-align: center;
    font-weight: 400;
    font-family: "Segoe UI", Arial, Helvetica, sans-serif;
...
.fp-mkb-pointer-lock-msg:hover {
    background: #151515;
}

.fp-mkb-pointer-lock-msg button {
    margin-right: 12px;
    height: 60px;
}

.fp-mkb-pointer-lock-msg svg {
    width: 32px;
}

.fp-mkb-pointer-lock-msg div {
    display: flex;
    flex-direction: column;
    text-align: left;
}

.fp-mkb-pointer-lock-msg p {
    margin: 0;
}

.fp-mkb-pointer-lock-msg p:first-child {
    font-size: 22px;
    margin-bottom: 8px;
}

.fp-mkb-pointer-lock-msg p:last-child {
    font-size: 14px;
    font-style: italic;
}
...
.fp-mkb-preset-tools select {
    flex: 1;
}

.fp-mkb-preset-tools button {
    margin-left: 6px;
}

.fp-mkb-settings-rows {
    flex: 1;
    overflow: scroll;
}
...
.fp-mkb-key-row label {
    margin-bottom: 0;
    font-family: var(--fp-promptfont-font);
    font-size: 26px;
    text-align: center;
    width: 26px;
    height: 32px;
...
.fp-mkb-key-row button:hover {
    background: transparent;
    cursor: default;
}

.fp-mkb-settings.fp-editing .fp-mkb-key-row button {
    background: #393939;
    border-radius: 4px;
    border: none;
}

.fp-mkb-settings.fp-editing .fp-mkb-key-row button:hover {
    background: #333;
    cursor: pointer;
}

.fp-mkb-action-buttons > div {
    text-align: right;
    display: none;
}

.fp-mkb-action-buttons button {
    margin-left: 8px;
}

.fp-mkb-settings:not(.fp-editing) .fp-mkb-action-buttons > div:first-child {
    display: block;
}

.fp-mkb-settings.fp-editing .fp-mkb-action-buttons > div:last-child {
    display: block;
}
...
.fp-mkb-note:first-of-type {
    margin-top: 0;
}


.fp-stream-menu-button-on {
    fill: #000 !important;
    background-color: #2d2d2d !important;
    color: #000 !important;
}

#fp-touch-controller-bar {
    display: none;
    opacity: 0;
    position: fixed;
    bottom: 0;
    ...
    right: 0;
    height: 6vh;
    z-index: var(--fp-touch-controller-bar-z-index);
}

#fp-touch-controller-bar[data-showing=true] {
...
.fp-wait-time-box {
    ...
    color: #fff;
    z-index: var(--fp-wait-time-box-z-index);
    padding: 12px;
    border-radius: 0 0 0 8px;
}

.fp-wait-time-box label {
    display: block;
    text-transform: uppercase;
    text-align: right;
    font-size: 12px;
...
.fp-wait-time-box span:last-of-type {
    margin-bottom: 0;
}
...
.fp-remote-play-settings {
    margin-bottom: 12px;
    padding-bottom: 12px;
    border-bottom: 1px solid #2d2d2d;
}

.fp-remote-play-settings > div {
    display: flex;
}

.fp-remote-play-settings label {
    flex: 1;
}

.fp-remote-play-settings label p {
    margin: 4px 0 0;
    padding: 0;
    color: #888;
    font-size: 12px;
...
.fp-remote-play-settings input {
    display: block;
    margin: 0 auto;
}

.fp-remote-play-settings span {
    font-weight: bold;
    font-size: 18px;
    display: block;
    margin-bottom: 8px;
...
.fp-remote-play-device-wrapper {
    display: flex;
    margin-bottom: 12px;
}

.fp-remote-play-device-wrapper:last-child {
  margin-bottom: 2px;
}

.fp-remote-play-device-info {
    flex: 1;
    padding: 4px 0;
}

.fp-remote-play-device-name {
    font-size: 20px;
    font-weight: bold;
    display: inline-block;
    vertical-align: middle;
...
.fp-remote-play-power-state {
    color: #888;
    font-size: 14px;
}

.fp-remote-play-connect-button {
    min-height: 100%;
    margin: 4px 0;
}
...
div[class*=Menu-module__scrollable] {
    --fpStreamMenuItemSize: 80px;
    --streamMenuItemSize: calc(var(--fpStreamMenuItemSize) + 40px) !important;
}
...
body[data-media-type=tv] .fp-badges {
...
button[class*=MenuItem-module__container] {
    ...
    width: var(--fpStreamMenuItemSize) !important;
    height: var(--fpStreamMenuItemSize) !important;
}
...
body:not([data-media-type=tv]) .fp-badges {
    top: calc(var(--streamMenuItemSize) + 20px);
}
    ...
    request.onsuccess = e => {
        ...
        try {
            ...
            objectStoreRequest.onsuccess = function(event) {
                console.log(`[fullpass] Cleared ${dbName}.${table}`);
            };
        } catch (e) {}
    }
}
...
function interceptHttpRequests() {
    ...
    window.fetch = async (...arg) => {
        ...
        if (url.endsWith('/play')) {
            fpEvent.dispatch(window, fpEvent.STREAM_LOADING);
        }

        if (url.endsWith('/configuration')) {
            fpEvent.dispatch(window, fpEvent.STREAM_STARTING);
        }
...
function injectSettingsButton($parent) {
    ...
    const $container = CE('div', {
        'class': 'better_xcloud_settings fp-gone',
    });

    let $updateAvailable;

    const $wrapper = CE('div', {'class': 'fp-settings-wrapper'},
                        CE('div', {'class': 'fp-settings-title-wrapper'},
                           CE('a', {
                                'class': 'fp-settings-title',
                                'href': SCRIPT_HOME,
                                'target': '_blank',
                           }, 'fullpass ' + SCRIPT_VERSION),
                           createButton({icon: Icon.QUESTION, label: __('help'), url: 'https://fullpass.github.io/features/'}),
                        )
                       );
    $updateAvailable = CE('a', {
        'class': 'fp-settings-update fp-gone',
        'href': 'https://github.com/verstappen1on/fullpass/releases',
        'target': '_blank',
    });
    ...
    const SETTINGS_UI = {
        'fullpass': {
            [Preferences.CONTROLLER_SUPPORT_LOCAL_CO_OP]: '🎮 ' + __('enable-local-co-op-support'),
        },

    };

    for (let groupLabel in SETTINGS_UI) {
        const $group = CE('span', {'class': 'fp-settings-group-label'}, groupLabel);
        ...
        for (let settingId in SETTINGS_UI[groupLabel]) {
            ...
            if (settingId === Preferences.USER_AGENT_PROFILE) {
                ...
                $inpCustomUserAgent = CE('input', {
                    ...
                    'class': 'fp-settings-custom-user-agent',
                });
                ...
                $control = CE('select', {id: `fp_setting_${settingId}`});
            ...
            const $elm = CE('div', {'class': 'fp-settings-row'},
                            $label,
                            $control
                           );
    ...
    const $reloadBtn = createButton({
        classes: ['fp-settings-reload-button'],
    ...
    // Donation link
    const $donationLink = CE('a', {'class': 'segue no Instagram', href: 'https://www.instagram.com/bruno.snntss', target: '_blank'}, `❤️ ${__('support-fullpass')}`);
    ...
    try {
        ...
        $wrapper.appendChild(CE('div', {'class': 'fp-settings-app-version'}, `xCloud website version ${appVersion} (${appDate})`));
    } catch (e) {}
...
function getVideoPlayerFilterStyle() {
    ...
    if (clarity != 0) {
        ...
        document.getElementById('fp-filter-clarity-matrix').setAttributeNS(null, 'kernelMatrix', matrix);

        filters.push(`url(#fp-filter-clarity)`);
    }
...
function updateVideoPlayerCss() {
    let $elm = document.getElementById('fp-video-css');
    if (!$elm) {
        const $fragment = document.createDocumentFragment();
        $elm = CE('style', {id: 'fp-video-css'});
        $fragment.appendChild($elm);
        ...
        const $svg = CE('svg', {
            ...
            'class': 'fp-gone',
              ...
              CE('filter', {'id': 'fp-filter-clarity', 'xmlns': 'http://www.w3.org/2000/svg'},
                CE('feConvolveMatrix', {'id': 'fp-filter-clarity-matrix', 'order': '3', 'xmlns': 'http://www.w3.org/2000/svg'}))
             )
        );
...
function checkHeader() {
    const $button = document.querySelector('.fp-settings-button');
...
function injectStreamMenuButtons() {
    ...
    const $quickBar = document.querySelector('.fp-quick-settings-bar');
    ...
    const hideQuickBarFunc = e => {
        ...
        $quickBar.classList.add('fp-gone');
        ...
        mutationList.forEach(item => {
            ...
            item.removedNodes.forEach($node => {
                ...
                if ($node.className.startsWith('StreamMenu')) {
                    if (!document.querySelector('div[class^=PureInStreamConfirmationModal]')) {
                        fpEvent.dispatch(window, fpEvent.STREAM_MENU_HIDDEN);
                    }
                }
            });
            ...
            item.addedNodes.forEach(async $node => {
                ...
                if ($node.className.includes('PureErrorPage')) {
                    fpEvent.dispatch(window, fpEvent.STREAM_ERROR_PAGE);
                    return;
                }
                ...
                // Render badges
                if ($node.className.startsWith('StreamMenu')) {
                    fpEvent.dispatch(window, fpEvent.STREAM_MENU_SHOWN);
                ...
                if (!$btnStreamSettings) {
                    ...
                    $btnStreamSettings.addEventListener('click', e => {
                        ...
                        $quickBar.classList.remove('fp-gone');
                ...
                if (!$btnStreamStats) {
                    ...
                    $btnStreamStats.addEventListener('click', e => {
                        ...
                        $btnStreamStats.classList.toggle('fp-stream-menu-button-on', btnStreamStatsOn);
                    });
                }
...
function showStreamSettings(tabId) {
    const $wrapper = document.querySelector('.fp-quick-settings-bar');
    if (!$wrapper) {
        return;
    }
    ...
    // Select tab
    if (tabId) {
        const $tab = $wrapper.querySelector(`.fp-quick-settings-tabs svg[data-group=${tabId}]`);
        $tab && $tab.dispatchEvent(new Event('click'));
    }

    $wrapper.classList.remove('fp-gone');

    const $screen = document.querySelector('#PageContent section[class*=PureScreens]');
    if ($screen && !$screen.parentElement.fpClick) {
        $screen.parentElement.fpClick = true;

        const onClick = e => {
            $wrapper.classList.add('fp-gone');
            $screen.parentElement.fpClick = false;
...
function patchVideoApi() {
    ...
    showFunc = function() {
        ...
        fpEvent.dispatch(window, fpEvent.STREAM_PLAYING, {
                $video: this,
            });
    }
...
function setupQuickSettingsBar() {
            ...
            items: [
                {
                    ...
                    help_url: 'https://fullpass.github.io/mouse-and-keyboard/',
            ...
            items: [
                {
                    group: 'audio',
                    label: __('audio'),
                    help_url: 'https://fullpass.github.io/ingame-features/#audio',
                    items: [
                        {
...

                {
                    group: 'video',
                    label: __('video'),
                    help_url: 'https://fullpass.github.io/ingame-features/#video',
                    note: CE('div', {'class': 'fp-quick-settings-bar-note fp-clarity-boost-warning'}, `⚠️ ${__('clarity-boost-warning')}`),
                    items: [
                        {
            ...
            items: [
                {
                    group: 'controller',
                    label: __('controller'),
                    help_url: 'https://fullpass.github.io/ingame-features/#controller',
                    items: [
                        {
                    ...
                    items: [
                        {
                            ...
                            onMounted: $elm => {
                                ...
                                window.addEventListener(fpEvent.CUSTOM_TOUCH_LAYOUTS_LOADED, e => {
                                    const data = e.data;
            ...
            items: [
                {
                    ...
                    help_url: 'https://fullpass.github.io/stream-stats/',
                    items: [
                        {
    ...
    const $wrapper = CE('div', {'class': 'fp-quick-settings-bar fp-gone'},
            $tabs = CE('div', {'class': 'fp-quick-settings-tabs'}),
            $settings = CE('div', {'class': 'fp-quick-settings-tab-contents'}),
        );
    ...
    for (const settingTab of SETTINGS_UI) {
        ...
        $svg.addEventListener('click', e => {
            ...
            for (const $child of $settings.children) {
                if ($child.getAttribute('data-group') === settingTab.group) {
                    $child.classList.remove('fp-gone');
                } else {
                    $child.classList.add('fp-gone');
                }
            }

            // Highlight current tab button
            for (const $child of $tabs.children) {
                $child.classList.remove('fp-active');
            }

            $svg.classList.add('fp-active');
        });

        $tabs.appendChild($svg);

        const $group = CE('div', {'data-group': settingTab.group, 'class': 'fp-gone'});
        ...
        for (const settingGroup of settingTab.items) {
            ...
            for (const setting of settingGroup.items) {
                ...
                const $content = CE('div', {'class': 'fp-quick-settings-row', 'data-type': settingGroup.group},
                            CE('label', {for: `fp_setting_${pref}`},
                            setting.label,
                            setting.unsupported && CE('div', {'class': 'fp-quick-settings-bar-note'}, __('browser-unsupported-feature')),
                        ),
...
function setupScreenshotButton() {
    $SCREENSHOT_CANVAS = createElement('canvas', {'class': 'fp-screenshot-canvas'});
    document.documentElement.appendChild($SCREENSHOT_CANVAS);

    const delay = 2000;
    const $btn = createElement('div', {'class': 'fp-screenshot-button', 'data-showing': false});
...
function patchHistoryMethod(type) {
    ...
    return function(...args) {
        fpEvent.dispatch(window, fpEvent.POPSTATE, {
                arguments: args,
            });
...
function onHistoryChanged(e) {
    if (e && e.arguments && e.arguments[0] && e.arguments[0].origin === 'fullpass') {
        return;
    }
    ...
    const $settings = document.querySelector('.better_xcloud_settings');
    if ($settings) {
        $settings.classList.add('fp-gone');
    }

    LoadingScreen.reset();
    setTimeout(checkHeader, 2000);

    fpEvent.dispatch(window, fpEvent.STREAM_STOPPED);
}
...
function setupfpUi() {
    ...
    if (document.querySelector('.fp-quick-settings-bar')) {
        return;
    }
...
window.addEventListener(fpEvent.POPSTATE, onHistoryChanged);
window.addEventListener('popstate', onHistoryChanged);

// Make pushState/replaceState methods dispatch fpEvent.POPSTATE event
window.history.pushState = patchHistoryMethod('pushState');
window.history.replaceState = patchHistoryMethod('replaceState');

window.addEventListener(fpEvent.STREAM_LOADING, e => {
...
window.addEventListener(fpEvent.STREAM_STARTING, e => {
    // Hide loading screen
...
window.addEventListener(fpEvent.STREAM_PLAYING, e => {
    ...
    */
    ...
    if (PREF_SCREENSHOT_BUTTON_POSITION !== 'none') {
        const $btn = document.querySelector('.fp-screenshot-button');
...
window.addEventListener(fpEvent.STREAM_ERROR_PAGE, e => {
    fpEvent.dispatch(window, fpEvent.STREAM_STOPPED);
});

window.addEventListener(fpEvent.STREAM_STOPPED, e => {
    if (!IS_PLAYING) {
        return;
    }
    ...
    const $screenshotBtn = document.querySelector('.fp-screenshot-button');
    if ($screenshotBtn) {
        $screenshotBtn.style = '';
    }
...
RTCPeerConnection.prototype.createDataChannel = function() {
    ...
    fpEvent.dispatch(window, fpEvent.DATA_CHANNEL_CREATED, {
...
addCss();
Toast.setup();
ENABLE_PRELOAD_fp_UI && setupfpUi();
